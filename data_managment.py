...
# %% Reading dataset
# Handle dataset
data_1 = pd.read_csv(r'4C_general_007.csv', header = None) # Fault diameter 0.007 in
data_2 = pd.read_csv(r'4C_general_014.csv', header = None) # Fault diameter 0.014 in
data_3 = pd.read_csv(r'4C_general_021.csv', header = None) # Fault diameter 0.021 in

data = pd.concat([data_1, data_2, data_3], ignore_index=True, axis=0) 
# %% Structural variables
size = 16*16 # Matrix dimmensions
n_sample = int(len(data)/size)

#%%  Normal dataset
n_data = data.values[0:n_sample*size,[0,4]] 
n_data = np.reshape(n_data, (n_sample,2,16,16)) # Matrix to tensor 2x16x16
